...
import { Modal, axiosInstance, eventEmitter, isDevEnv } from "../../utils";
		...
		bankLogs: [
			{
				...
				action: "withdraw",
				amount: 1000,
				date: moment().subtract(7, "minutes").toDate().getTime()
			},
			{
				author: "HeyyCzer",
				action: "deposit",
				amount: 5000,
				date: moment().subtract(10, "minutes").toDate().getTime()
			},
		],
	} : {
		balance: 0,
		...
		return (
			<App>
				<div className="flex flex-col justify-center items-center h-full w-2/3 mx-auto text-center">
					...
					<h1 className="text-black/70 dark:text-white/70 text-sm">Você não possui acesso à essa página!</h1>
				</div>
			</App>
		)
	...
	return (
		<App>
			...
			<div className="mx-3">
				{/* Bank */}
				<section>
					<div className="px-5 pt-3 pb-4 bg-gray-100 dark:bg-dark-3 rounded-lg w-full">
						<span className="text-xs text-black/50 dark:text-white/50 tracking-wide">Saldo disponível</span>
						<h1 className="text-2xl leading-none tracking-wider">$ { formatter.format(company.balance) }</h1>
					</div>
					...
					{/* Online */}
					<div className="my-4 bg-gray-100 dark:bg-dark-3 rounded-lg px-5 py-3">
						{/* Title */}
						...
						<div className="my-2">
							{
								company.workers.length === 0 ?
									<div className="text-center text-xs">Nenhum funcionário</div> :
					...
					<div className="my-4 bg-gray-100 dark:bg-dark-3 rounded-lg px-5 py-3">
						...
						<div className="my-2">
							{
								company.bankLogs.length === 0 ? <div className="text-center text-xs">Nenhum histórico até o momento</div> :
